...
//		music = Mix_LoadMUS("data/music/title.mid");
	...
	while ( gamestate == GAMESTATE_GAME )
	{
        ...
        // colorbars
        int t_life;
        if (p_life[0] > 0) { t_life = p_life[0]; if (t_life > 66) t_life = 66;
        t_rect.w = t_life;
        t_rect.x = 0;
        SDL_FillRect(screen,&t_rect,SDL_MapRGB(screen->format,255,0,0));
        t_life = p_life[0] - 66; if (t_life > 0) {
        t_rect.w = t_life;
        t_rect.x = 66;
        ...
        if (p_life[1] > 0) {
        t_life = p_life[1]; if (t_life > 66) t_life = 66;
        t_rect.x = 640 - t_life;
        t_rect.w = t_life;
        SDL_FillRect(screen,&t_rect,SDL_MapRGB(screen->format,255,0,0));
        t_life = p_life[1] - 66; if (t_life > 0) {
        t_rect.x = 574 - t_life;
        t_rect.w = t_life;
        SDL_FillRect(screen,&t_rect,SDL_MapRGB(screen->format,0,255,0));
        } }
...
//                    for (i=0; i<2; i++)
                    {
                        ...
                        if (stun[i] > 0) stun[i] --; else {
                            ...
                            if (chargin[i] == 1)
                            {
                                cooldown[i] = 55;
                                p_act[i] = 5;
                                if (dist < 16) { stun[opp] = 50;  p_act[opp] = 3; p_life[opp] -= 33; chargin[opp] = 0;
                                    if (vol_sfx) { Mix_PlayChannel(-1,sfx_hit[rand()%5],0); }
                                }
                            }
                            // decrement chargin
                            if (chargin[i]) chargin[i] --; else {

                                if (jumpin[i] == 0 && duckin[i] == 0) {
                                    if (dxkeys[0+ (i * 7)]) {
                                        jumpin[i] = 1;
                                        if (vol_sfx) { Mix_PlayChannel(-1,sfx_jump,0); }
                                    } else if (dxkeys[1+ (i * 7)]) {
                                        duckin[i] = 1;
                                        if (vol_sfx) { Mix_PlayChannel(-1,sfx_duck,0); }
                                    }
                                }
                                ...
                                if (cooldown[i] > 0) cooldown[i] --; else {
                                    if (dxkeys[4 + (i * 7)]) {
                                        cooldown[i] = 30;
                                        p_act[i] = 1;
                                        if (duckin[opp] == 0 && dist < 16) { stun[opp] = 25;  p_act[opp] = 3; p_life[opp] -= 16; chargin[opp] = 0;
                                            if (vol_sfx) { Mix_PlayChannel(-1,sfx_hit[rand()%5],0); }
                                        }
                                    } else if (dxkeys[5 + (i * 7)]) {
                                        cooldown[i] = 30;
                                        p_act[i] = 2;
                                        if (jumpin[opp] == 0 && dist < 16) { stun[opp] = 25; p_act[opp] = 3; p_life[opp] -= 16; chargin[opp] = 0;
                                            if (vol_sfx) { Mix_PlayChannel(-1,sfx_hit[rand()%5],0); }
                                        }
                                    } else if (dxkeys[6 + (i * 7)] && !jumpin[i] && !duckin[i]) {
                                    	// no air specials : )
                                        if (vol_sfx) { Mix_PlayChannel(-1,sfx_charge,0); }
                                        chargin[i] = 32;
                                    }
                                }
                            }
                    ...
                    // check life
                    if (p_life[0] <= 0) { p_life[0] = 0; p_act[0] = 4; p_rect_dst[0].w=p_rect_src[0].w=128; p_rect_dst[0].h = p_rect_src[0].h = 64; p_rect_dst[0].y = center_screen.y + center_screen.h - 64; playing = 0; winner = 1;
                        if (vol_sfx) { Mix_PlayChannel(-1,sfx_win[arena_id],0); }}
                    else if (p_life[1] <= 0) { p_life[1] = 0; p_act[1] = 4; p_rect_dst[1].w=p_rect_src[1].w=128; p_rect_dst[1].h = p_rect_src[1].h=64; p_rect_dst[1].y = center_screen.y + center_screen.h - 64; playing = 0; winner = 0;
                        if (vol_sfx) { Mix_PlayChannel(-1,sfx_win[arena_id],0); }}


                    if (dist < 0) { int q = dist / 2; p_rect_dst[0].x += q; p_rect_dst[1].x -= q;}
                ...
                {
                    last_ticks += 500;
                    p_life[0] ++; if (p_life[0] > 266) p_life[0] = 266;
                    p_life[1] ++; if (p_life[1] > 266) p_life[1] = 266;
                }
            }
        } else {
        }
...
#define key(x) (event.key.keysym.sym == gamekeys[x])
				case SDL_KEYUP:
                    ...
                    {
                        if (key(0) || key(1)) { p_pos[0]=0; p_rect_dst[0].y=center_screen.y+center_screen.h-128; p_rect_src[0].h=128; }
                        else if (key(4) || key(5) || key(6)) { p_act[0] = 0; if (p_act[1] == 3) { p_act[1] = 0; } }
                        else if (key(7) || key(8)) { p_pos[1]=0; p_rect_dst[1].y=center_screen.y+center_screen.h-128; p_rect_src[1].h=128; }
                        else if (key(11) || key(12) || key(13)) { p_act[1] = 0; if (p_act[0] == 3) { p_act[0] = 0; } }
                    } else {
                        if key(0) dxkeys[0] = 0;
...
// L, R, U, D, P, K, S
                    if (!dx_mode)
                    {
                        if (playing)
                        {
                            if key(0) { p_pos[0] = 1; p_rect_dst[0].y=center_screen.y+center_screen.h-160; p_rect_src[0].h=128;}
                            else if key(1) { p_pos[0] = 2; p_rect_dst[0].y=center_screen.y+center_screen.h-96; p_rect_src[0].h=96;}
                            else if key(2) { p_rect_dst[0].x -= 8; }
                            else if key(3) { p_rect_dst[0].x += 8; }
                            else if key(4) { p_act[0] = 1; if (p_pos[1] != 2 && dist < 16) { p_life[1] -= 4; p_act[1] = 3; } }
                            else if key(5) { p_act[0] = 2;  if (p_pos[1] != 1 && dist < 16) { p_life[1] -= 4; p_act[1] = 3; } }
                            else if (key(6) && (p_choice[0] > 9)) { p_act[0] = 5; if (dist < 16) { p_life[1] -= 8; p_act[1] = 3; } }

                            else if key(7) { p_pos[1] = 1; p_rect_dst[1].y=center_screen.y+center_screen.h-160; p_rect_src[1].h=128;}
                            else if key(8) { p_pos[1] = 2; p_rect_dst[1].y=center_screen.y+center_screen.h-96; p_rect_src[1].h=96;}
                            else if key(9) { p_rect_dst[1].x -= 8; }
                            else if key(10) { p_rect_dst[1].x += 8; }
                            else if key(11) { p_act[1] = 1; if (p_pos[0] != 2 && dist < 16) { p_life[0] -= 4; p_act[0] = 3; } }
                            else if key(12) { p_act[1] = 2; if (p_pos[0] != 1 && dist < 16) { p_life[0] -= 4; p_act[0] = 3; } }
                            else if (key(13) && (p_choice[1] > 9)) { p_act[1] = 5; if (dist < 16) { p_life[0] -= 8; p_act[0] = 3; } }
                        }

                        // check life
                        if (p_life[0] <= 0) { p_life[0] = 0; p_act[0] = 4; p_rect_dst[0].w=p_rect_src[0].w=128; p_rect_dst[0].h = p_rect_src[0].h = 64; p_rect_dst[0].y = center_screen.y + center_screen.h - 64; playing = 0; winner = 1; }
                        if (p_life[1] <= 0) { p_life[1] = 0; p_act[1] = 4; p_rect_dst[1].w=p_rect_src[1].w=128; p_rect_dst[1].h = p_rect_src[1].h=64; p_rect_dst[1].y = center_screen.y + center_screen.h - 64; playing = 0; winner = 0; }

                        // bump apart
                        while (dist < 0) { p_rect_dst[0].x -= 4; p_rect_dst[1].x += 4; }
