...
SDL_Surface *flipHorizontally( SDL_Surface* image, int colorkey )
{
    ...
    if (flipped_image == NULL) { printf( "Unable to flip image: %s\n", SDL_GetError() );
        gamestate = GAMESTATE_DONE;
        return NULL; }
...
#define QUICKLOAD_UI(NAME,TRANS) if( ! ( NAME = load_image("data/img/ui/" #NAME ".png",TRANS))) return;
    ...
    if (!tunes[SNG_TITLE]) { printf("Mix_LoadMUS(data/music/ui/intro.mid): %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    tunes[SNG_OPTIONS] = Mix_LoadMUS("data/music/ui/slowsong.mid");
    if (!tunes[SNG_OPTIONS]) { printf("Mix_LoadMUS(data/music/ui/slowsong.mid): %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    tunes[SNG_CREDITS] = Mix_LoadMUS("data/music/ui/Theme.mid");
    if (!tunes[SNG_CREDITS]) { printf("Mix_LoadMUS(data/music/ui/Theme.mid): %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    tunes[SNG_DXCREDITS] = Mix_LoadMUS("data/music/ui/cchitcat.mid");
    if (!tunes[SNG_DXCREDITS]) { printf("Mix_LoadMUS(data/music/ui/cchitcat.mid): %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    tunes[SNG_SELECT] = Mix_LoadMUS("data/music/ui/coldness.mid");
    if (!tunes[SNG_SELECT]) { printf("Mix_LoadMUS(data/music/ui/coldness.mid): %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    tunes[SNG_DXSELECT] = Mix_LoadMUS("data/music/ui/select.mid");
    if (!tunes[SNG_DXSELECT]) { printf("Mix_LoadMUS(data/music/ui/select.mid): %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    tunes[SNG_S_SELECT] = Mix_LoadMUS("data/music/ui/scared so.mid");
    if (!tunes[SNG_S_SELECT]) { printf("Mix_LoadMUS(data/music/ui/scared so.mid): %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    tunes[SNG_S_DXSELECT] = Mix_LoadMUS("data/music/ui/not1.mid");
    if (!tunes[SNG_S_DXSELECT]) { printf("Mix_LoadMUS(data/music/ui/not1.mid): %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }

    for (i=0; i<NUM_FIGHTSONGS; i++)
    {
        sprintf(buffer,"data/music/fight/%s.mid",fight_music[i]);
        tunes[i + SNG_FIGHT] = Mix_LoadMUS(buffer);
        if (!tunes[i + SNG_FIGHT]) { printf("Mix_LoadMUS(%s): %s\n",buffer,Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    }

    // load sfx
    if (!(sfx_charge = Mix_LoadWAV("data/sfx/charge.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    if (!(sfx_duck = Mix_LoadWAV("data/sfx/duck.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    if (!(sfx_jump = Mix_LoadWAV("data/sfx/jump.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    if (!(sfx_win[0] = Mix_LoadWAV("data/sfx/win1.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    if (!(sfx_win[1] = Mix_LoadWAV("data/sfx/win2.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    if (!(sfx_hit[0] = Mix_LoadWAV("data/sfx/hit1.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    if (!(sfx_hit[1] = Mix_LoadWAV("data/sfx/hit2.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    if (!(sfx_hit[2] = Mix_LoadWAV("data/sfx/hit3.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    if (!(sfx_hit[3] = Mix_LoadWAV("data/sfx/hit4.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
    if (!(sfx_hit[4] = Mix_LoadWAV("data/sfx/hit5.wav"))) { printf("Mix_LoadChunk: %s\n",Mix_GetError()); gamestate=GAMESTATE_DONE; return; }
}
...
#define CHUNKFREE(NAME) if (NAME != NULL) { Mix_FreeChunk (NAME); NAME = NULL; }
    ...
    {
        if (tunes[i] != NULL) { Mix_FreeMusic(tunes[i]); tunes[i] = NULL; }
    }
}
