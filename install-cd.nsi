...
; Check to see if already installed
  IfFileExists "$INSTDIR\SlugFest97DX.exe" 0 NotInstalled
  MessageBox MB_YESNO "SlugFest '97 DX is already installed, do you wish to play?" \
    IDNO Abort
  Exec $INSTDIR\SlugFest97DX.exe
Abort:
  Abort
...
Section "SlugFest '97 DX (required)"
  ...
  CopyFiles /SILENT /FILESONLY "$EXEDIR\readme.html" "$INSTDIR\readme.html" 10

  ; Data files follow
  CopyFiles /SILENT "$EXEDIR\data" "$INSTDIR" 771
...
Section "SlugFest '97 (Classic)"
  ...
  ; Data files follow
  CopyFiles /SILENT "$EXEDIR\classic" $INSTDIR 2114
...
Section "'Making Of' Video"
  ...
  ; Data files follow
  CopyFiles /SILENT /FILESONLY "$EXEDIR\makingof.mpg" $INSTDIR 1137
